...
class Connectors {
  ...
  constructor(config = {}) {
     ...
     * @return {Promise<object>} A Promise that resolves with the response object. The response payload is in the `data` property, which will be a JSON object or a Blob.
     */
    this.projects.locations.connections.executeHttpRequest = async (apiParams = {}, clientConfig = {}) => this._makeRequest('v2/{+name}:executeHttpRequest', 'POST', apiParams, clientConfig);
     ...
     * @param {string} apiParams.parent - (Required) Required. Resource name of the Connection. Format: projects/{project}/locations/{location}/connections/{connection}
     * @param {string} apiParams.toolNames - List of tool names for selective tool fetching.
     * @param {object} [clientConfig] - Optional client-side configuration.
     * @param {string} [clientConfig.responseType] - The expected response type. Setting to 'blob' returns the raw file content. Omit for JSON.
     * @return {Promise<object>} A Promise that resolves with the response object. The response payload is in the `data` property, which will be a JSON object or a Blob.
     */
