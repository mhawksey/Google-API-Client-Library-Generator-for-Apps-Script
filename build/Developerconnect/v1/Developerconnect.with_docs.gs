...
class Developerconnect {
  ...
  constructor(config = {}) {
    ...
    this.projects.locations.insightsConfigs.deploymentEvents = {};

    /**
     * Gets a single Deployment Event.
     * @param {object} apiParams - The parameters for the API request.
     * @param {string} apiParams.name - (Required) Required. The name of the deployment event to retrieve. Format: projects/{project}/locations/{location}/insightsConfigs/{insights_config}/deploymentEvents/{uuid}
     * @param {object} [clientConfig] - Optional client-side configuration.
     * @param {string} [clientConfig.responseType] - The expected response type. Setting to 'blob' returns the raw file content. Omit for JSON.
     * @return {Promise<object>} A Promise that resolves with the response object. The response payload is in the `data` property, which will be a JSON object or a Blob.
     */
    this.projects.locations.insightsConfigs.deploymentEvents.get = async (apiParams = {}, clientConfig = {}) => this._makeRequest('v1/{+name}', 'GET', apiParams, clientConfig);
     ...
     * @return {Promise<object>} A Promise that resolves with the response object. The response payload is in the `data` property, which will be a JSON object or a Blob.
     */
    this.projects.locations.insightsConfigs.deploymentEvents.list = async (apiParams = {}, clientConfig = {}) => this._makeRequest('v1/{+parent}/deploymentEvents', 'GET', apiParams, clientConfig);
     ...
     * @param {string} apiParams.googleOauthParams.scopes - Required. The scopes returned by Google OAuth flow.
     * @param {string} apiParams.googleOauthParams.ticket - Required. The ticket to be used for post processing the callback from Google OAuth flow.
     * @param {string} apiParams.googleOauthParams.versionInfo - Optional. The version info returned by Google OAuth flow.
     * @param {string} apiParams.oauthParams.code - Required. The code to be used for getting the token from SCM provider.
     * @param {string} apiParams.oauthParams.ticket - Required. The ticket to be used for post processing the callback from SCM provider.
     * @param {object} [clientConfig] - Optional client-side configuration.
     * @param {string} [clientConfig.responseType] - The expected response type. Setting to 'blob' returns the raw file content. Omit for JSON.
     * @return {Promise<object>} A Promise that resolves with the response object. The response payload is in the `data` property, which will be a JSON object or a Blob.
     */
    this.projects.locations.accountConnectors.users.finishOAuthFlow = async (apiParams = {}, clientConfig = {}) => this._makeRequest('v1/{+accountConnector}/users:finishOAuthFlow', 'GET', apiParams, clientConfig);
     ...
     * @return {Promise<object>} A Promise that resolves with the response object. The response payload is in the `data` property, which will be a JSON object or a Blob.
     */
    this.projects.locations.accountConnectors.users.startOAuthFlow = async (apiParams = {}, clientConfig = {}) => this._makeRequest('v1/{+accountConnector}/users:startOAuthFlow', 'GET', apiParams, clientConfig);
