...
class Merchantapi {
  ...
  constructor(config = {}) {
     ...
     * @param {string} params.dataSource - Required. The primary or supplemental product data source name. If the product already exists and data source provided is different, then the product will be moved to a new data source. For more information, see [Overview of Data sources sub-API](/merchant/api/guides/data-sources/overview). Only API data sources are supported. Format: `accounts/{account}/dataSources/{datasource}`. For example, `accounts/123456/dataSources/104628`.
     ...
     */
    this.accounts.productInputs.insert = (params) => this._makeRequest('products/v1beta/{+parent}/productInputs:insert', 'POST', params);
     ...
     * @return {object} The API response object.
     */
    this.accounts.productInputs.patch = (params) => this._makeRequest('products/v1beta/{+name}', 'PATCH', params);
     ...
     * @return {object} The API response object.
     */
    this.accounts.products.list = (params) => this._makeRequest('products/v1beta/{+parent}/products', 'GET', params);
  }
