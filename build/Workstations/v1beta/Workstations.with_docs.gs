...
class Workstations {
  ...
  constructor(config = {}) {
     ...
     * @param {object} apiParams - The parameters for the API request.
     * @param {string} apiParams.workstation - (Required) Required. Name of the workstation for which the credentials should be pushed.
     * @param {object} apiParams.requestBody - The request body.
     * @param {object} [clientConfig] - Optional client-side configuration.
     * @param {string} [clientConfig.responseType] - The expected response type. Setting to 'blob' returns the raw file content. Omit for JSON.
     * @return {Promise<object>} A Promise that resolves with the response object. The response payload is in the `data` property, which will be a JSON object or a Blob.
     */
    this.projects.locations.workstationClusters.workstationConfigs.workstations.pushCredentials = async (apiParams = {}, clientConfig = {}) => this._makeRequest('v1beta/{+workstation}:pushCredentials', 'POST', apiParams, clientConfig);
