...
class Cloudscheduler {
  ...
  constructor(config = {}) {
     ...
     * @param {string} params.pageToken - A token identifying a page of results the server will return. To request the first page results, page_token must be empty. To request the next page of results, page_token must be the value of next_page_token returned from the previous call to ListJobs.
     ...
     * @return {object} The API response object.
     */
    this.projects.locations.jobs.patch = (params) => this._makeRequest('v1/{+name}', 'PATCH', params);
     ...
     * @return {object} The API response object.
     */
    this.projects.locations.jobs.pause = (params) => this._makeRequest('v1/{+name}:pause', 'POST', params);
     ...
     * @return {object} The API response object.
     */
    this.projects.locations.jobs.resume = (params) => this._makeRequest('v1/{+name}:resume', 'POST', params);
     ...
     * @return {object} The API response object.
     */
    this.projects.locations.jobs.run = (params) => this._makeRequest('v1/{+name}:run', 'POST', params);
