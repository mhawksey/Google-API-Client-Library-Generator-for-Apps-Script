...
/**
 * Google Apps Script client library for the Parallelstore API
 * Documentation URL: https://cloud.google.com/parallelstore
 * @class
 */
class Parallelstore {
  /**
  ...
  constructor(config = {}) {
    ...
    this._rootUrl = 'https://parallelstore.googleapis.com/';
    this._servicePath = '';
     ...
     * @param {string} params.parent - (Required) Required. The project and location for which to retrieve instance information, in the format `projects/{project_id}/locations/{location}`. To retrieve instance information for all locations, use "-" as the value of `{location}`.
     * @return {object} The API response object.
     */
     ...
     * @param {string} params.instanceId - Required. The name of the Parallelstore instance. * Must contain only lowercase letters, numbers, and hyphens. * Must start with a letter. * Must be between 1-63 characters. * Must end with a number or a letter. * Must be unique within the customer project / location
     * @param {string} params.parent - (Required) Required. The instance's project and location, in the format `projects/{project}/locations/{location}`. Locations map to Google Cloud zones; for example, `us-west1-b`.
     ...
     * @param {string} params.name - (Required) Identifier. The resource name of the instance, in the format `projects/{project}/locations/{location}/instances/{instance_id}`.
     * @param {string} params.requestId - Optional. An optional request ID to identify requests. Specify a unique request ID so that if you must retry your request, the server will know to ignore the request if it has already been completed. The server will guarantee that for at least 60 minutes since the first request. For example, consider a situation where you make an initial request and the request times out. If you make the request again with the same request ID, the server can check if original operation with the same request ID was received, and if so, will ignore the second request. This prevents clients from accidentally creating duplicate commitments. The request ID must be a valid UUID with the exception that zero UUID is not supported (00000000-0000-0000-0000-000000000000).
     * @param {string} params.updateMask - Required. Mask of fields to update. Field mask is used to specify the fields to be overwritten in the Instance resource by the update. At least one path must be supplied in this field. The fields specified in the update_mask are relative to the resource, not the full request.
     * @param {object} params.resource - The request body.
     * @return {object} The API response object.
     */
    this.projects.locations.instances.patch = (params) => this._makeRequest('v1beta/{+name}', 'PATCH', params);
     ...
     * @return {object} The API response object.
     */
    this.projects.locations.instances.importData = (params) => this._makeRequest('v1beta/{+name}:importData', 'POST', params);
     ...
     * @return {object} The API response object.
     */
    this.projects.locations.instances.exportData = (params) => this._makeRequest('v1beta/{+name}:exportData', 'POST', params);
  }
