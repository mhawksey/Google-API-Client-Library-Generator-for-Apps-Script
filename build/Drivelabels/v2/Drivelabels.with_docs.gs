...
class Drivelabels {
  ...
  constructor(config = {}) {
     ...
     * @param {string} params.customer - The customer to scope this request to. For example: `customers/abcd1234`. If unset, it will return settings within the current customer.
     ...
     * @param {string} params.writeControl.requiredRevisionId - The revision ID of the label that the write request will be applied to. If this isn't the latest revision of the label, the request will not be processed and will return a 400 Bad Request error.
     * @return {object} The API response object.
     */
     ...
     * @param {string} params.pageToken - The token of the page to return.
     * @param {string} params.parent - (Required) Required. The parent label resource name on which label permissions are listed. Format: `labels/{label}`.
     * @param {boolean} params.useAdminAccess - Set to `true` in order to use the user's admin credentials. The server will verify the user is an admin for the label before allowing access.
     * @return {object} The API response object.
     */
