...
- **Last Checked:** Thu, 30 Apr 2026 23:31:58 GMT
- **Last Modified:** Thu, 30 Apr 2026 23:31:58 GMT
...
#### `projects.instances.clusters.getMemoryLayer()`

Gets information about the memory layer of a cluster.

| Parameter | Type | Required | Description |
|---|---|---|---|
| `params.name` | `string` | Yes | Required. The unique name of the requested cluster's memory layer. Values are of the form `projects/{project}/instances/{instance}/clusters/{cluster}/memoryLayer`. |

### `projects.instances.clusters.memoryLayers`

#### `projects.instances.clusters.memoryLayers.list()`

Lists information about memory layers.

| Parameter | Type | Required | Description |
|---|---|---|---|
| `params.parent` | `string` | Yes | Required. The unique name of the cluster for which a list of memory layers is requested. Values are of the form `projects/{project}/instances/{instance}/clusters/{cluster}`. Use `{cluster} = '-'` to list MemoryLayers for all Clusters in an instance, e.g., `projects/myproject/instances/myinstance/clusters/-`. |
| `params.pageSize` | `integer` | No | Optional. The maximum number of memory layers to return. The service may return fewer than this value. |
| `params.pageToken` | `string` | No | Optional. A page token, received from a previous `ListMemoryLayers` call. Provide this to retrieve the subsequent page. When paginating, all other parameters provided to `ListMemoryLayers` must match the call that provided the page token. |
...
| `params.view` | `string` | No | Optional. Describes which of the materialized view's fields should be populated in the response. Defaults to SCHEMA_VIEW. |
...
| `params.view` | `string` | No | Optional. Describes which of the materialized view's fields should be populated in the response. For now, only the default value SCHEMA_VIEW is supported. |
